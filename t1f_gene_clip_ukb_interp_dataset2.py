...
class MRIandGenedataset(Dataset):
    def __init__(self,label=-1,fold=0,k=5,phase="train", data_path="/data/qinfeng/datasets/", gene_path="/data/qinfeng/datasets/UKB/UKB_all_array_completed_imputed/",age_range=None,task=None): #ADNIALL_SNPs ADNIALL_PPMI_IMMUNO_fuse
        ...
        self.data_path2="/data/qinfeng/datasets/UKB/CLIP_f_cs1000/"#KLAE_MRI_f AE_MRI_f CLIP_f
        ...
        sub_selected = pd.read_csv("ukb_sub_selected.csv").values[:,0].tolist()
        self.subject_list = [sub  for sub in self.subject_list  if (sub in sub_selected)]
        self.subject_list.sort()
        self.files = []
        self.label = []
        for file in os.listdir(self.path2):
            sub =  file.split('.')[0]
            if sub in self.subject_list[len(self.subject_list)//2:]:
